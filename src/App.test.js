...
// 1. App renders by default and loads initial api data without errors
// 2. Test response to user input with select
// 3. Test responses to state change
//  fetchShow api call is resolved successfully
//  fetchShow api call is rejected with error message

// test App component loaded, fetchShows API call for initial data is successful
test("App component renders and fetches initial data from useEffect hook without error", async () => {
  await act(() => {
    render(<App />);
...
// test("Dropdown renders and opens on user click", async () => {
//     mockFetchShow.mockImplementation(() => Promise.resolve(showData));
//     await act(() => {
//         render(<App />);
//         return Promise.resolve();
//       });
...
//       screen.debug();
//       // how to click the correct DOM element to open dropdown
//       fireEvent.click(menu);

//       await waitFor(() => {
//           const menuChoice = screen.getByText(/season 1/i);
//       })
//       screen.debug();    
// });
